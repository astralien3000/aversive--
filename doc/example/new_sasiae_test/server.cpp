#include <QThread>
#include <QProcess>
#include <cstring>
#include <iostream>

#define BUFFER_SIZE 1024

char buffer[BUFFER_SIZE];

// Basic function to get the next message recieved
bool readLine(QProcess& client) {
  unsigned int i = 0;
  char c;
  if(!client.getChar(&c)) {
    client.waitForReadyRead();
  }
  else {
    buffer[i++] = c;
  }
  while(client.getChar(&c) && c != '\n') {
    buffer[i++] = c;
  }
  buffer[i] = '\0';
  if(c == '\n') {
    return true;
  }
  else {
    client.kill();
    client.waitForFinished();
    return false;
  }
}

// Check if the next message recieved from the client is equal to str
bool checkMsg(QProcess& client, const char* str) {
  if(!readLine(client)) {
    return false;
  }
  return !strcmp(buffer, str);
}

// The result displayed after executing the test
void res(bool ok, int i = 0) {
  if(ok) {
    std::cout << "OK" << std::endl;
  }
  else {
    std::cout << "NOK " << i << std::endl;
  }
}

// Main function : write your test
int main(int argc, char** argv) {
  (void) argc;
  (void) argv;

  // Execute the client in an other process
  QProcess client;
  client.start("./client", QStringList());
  if(!client.waitForStarted()) {
    res(false, 1);
    return EXIT_FAILURE;
  }
  
  ////
  // Write your test here
  //// Example
  // client.write("T 1 10\n");
  // if(!checkMsg(client, "T")) {
  //   res(false, 12);
  //   return EXIT_FAILURE;
  // }


  // Check if the client ended correctly
  client.write("S\n");

  if(!checkMsg(client, "S")) {
    res(false, 114);
    return EXIT_FAILURE;
  }

  client.closeWriteChannel();
  
  if(!client.waitForFinished()) {
    res(false, 115);
    return EXIT_FAILURE;
  }

  res(true);
  return EXIT_SUCCESS;
}
